...
/**
 * Pub/sub channel helpers + an EventBus publisher. Moved here from the API
 * (apps/api/src/lib/events.ts) as part of the matcher cutover: the matcher is
 * now the sole owner of in-memory books, so it owns the derived-state
 * broadcast too. The API only *subscribes* to these channels (see
 * apps/api/src/routes/sse.ts) — the channel strings must stay in lockstep.
 */
...
/** Plain Redis key holding the latest JSON book snapshot for a market. */
 ...
 * logged but never thrown, so a flaky Redis can't take down the consumer loop.
 */
