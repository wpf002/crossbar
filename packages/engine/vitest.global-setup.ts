...
const dbPackageDir = resolve(here, '..', 'db');
...
function ensureTestDatabase(): void {
  const check = spawnSync(
    ...
    [
      ...
      'postgres',
      '-tAc',
      "SELECT 1 FROM pg_database WHERE datname='crossbar_test'",
    ],
    { encoding: 'utf8' },
  );
...
export async function teardown(): Promise<void> {
  // Leave the test DB around for inspection — wipe happens per-test.
}
